...
import { deleteWorkshopImage, getWorkshopImages, resetWorkshopImageState } from '@/app/Features/WorkshopImage/WorkshopImageSlice'
import TwoStepBreadcrumbs from '../../AdminComponents/BreadCrumbs/TwoStepBreadcrumbs'

const WorkshopImagesList = (workshop) => {
    ...
    const { workshopImages, responseStatus, responseMessage } = useSelector((state) => state.workshopImages)
    ...
    const fetchWorkshopImages = () => {
        setIsLoading(true)
        dispatch(getWorkshopImages(workshopID))
    }

    const filteredData = useMemo(() => {
        const filteredSlice = data.filter((item) => (
            item.workshopId?.toLowerCase()?.includes(searchTerm.toLowerCase())
        ));
    ...
    const handleDelete = (id) => {
        ...
        if (shouldDelete) {
            setIsLoading(true);
            dispatch(deleteWorkshopImage(id));
        }
    };
    ...
    useEffect(()=>{
        if ((responseStatus == 'success' && responseMessage == 'Workshop Image deleted successfully')) {
            setIsLoading(false)
            setshowMessageModal(true)
        }
        if ((responseStatus == 'success' && responseMessage == '')) {
            setIsLoading(false)
            setData(workshopImages)
        }
    ...
    return (
        <>
            <TwoStepBreadcrumbs 
                currentPageName='Workshop Images'
                previousPageName='Workshop Managment'
                previousPageLink='/admin/workshop'
            />
            <div className='bodySection'>
                <div className='d-flex align-items-center justify-content-between'>
                    <div className='fs-5'>Workshop Images</div>
                </div>
                <div className="row">
                                            ...
                                            <div className="row">
                                                <div className="col-md-3 mt-4">
                                                    <div className='card-body-custom'>
                                                        ...
                                                        <center>
                                                            <img src={`/upload/${val?.workshopImage}`} className='rounded max-width-100 max-height-250px' alt="" />
                                                        </center>
                                                    </div>
                                                </div>
                                            </div>
...
export default WorkshopImagesList